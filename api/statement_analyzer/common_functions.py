...
import openpyxl
from openpyxl.styles import Alignment
from statement_analyzer.utils import get_base_dir
import fitz
...
import json
from statement_analyzer.code_for_extraction import (
    extract_text_from_pdf,
    extract_with_test_cases,
    model_for_pdf,
...
def extract_account_details(text):
    ...
    try:
        ...
        names = [name.strip() for name in names if name.strip()]  # Clean up names list
        ...
        print(f"An error occurred while extracting names and account numbers: {str(e)}")
        return ["_", "XXXXXXXXXX"]
...
def extraction_process(bank, pdf_path, pdf_password, start_date, end_date):
    ...
    try:
        if ext == ".pdf":
            idf, text, explicit_lines = extract_with_test_cases(
...
def extraction_process_explicit_lines(
    bank, pdf_path, pdf_password, start_date, end_date, explicit_lines, labels
):
    CA_ID = "1234_temp"
    empty_idf = pd.DataFrame()
    default_name_n_num = ["_", "XXXXXXXXXX"]
    ...
    try:
        ...
        if not all_null:
            ...
            df.loc[-1] = (
                new_row  # Add the new row with a negative index to place it at the top
            )
            df.index = df.index + 1  # Shift all indices by 1
            df.sort_index(
                inplace=True
            )  # Reorder the DataFrame to update the row positions
...
def eod(df_original):
    ...
    if df.empty:
        return pd.DataFrame()  # Handle empty df after NaT drop
    df["Balance"] = pd.to_numeric(df["Balance"], errors="coerce")
    df.dropna(
        subset=["Balance"], inplace=True
    )  # Option: Drop rows with invalid balances
    if df.empty:
        return pd.DataFrame()  # Handle empty df after balance cleaning
    df.sort_values(
        by=["Bank", "Value Date"], inplace=True
    )  # Sort by Bank too for consistency
    df["YearMonth"] = df["Value Date"].dt.strftime(
        "%Y%m"
    )  # Keep as string initially for easier date range generation
    ...
    for bank in bank_names:
        ...
        if idf.empty:
            continue  # Skip banks with no valid data
        ...
        processed_months_strs_bank = (
            []
        )  # Keep track of month strings in order for THIS bank
        for month_start in all_month_starts:
            current_ym_str = month_start.strftime("%Y%m")
            current_month_str = month_start.strftime("%b-%Y")
            processed_months_strs_bank.append(
                current_month_str
            )  # Add to ordered list for pivoting
            all_banks_processed_months.add(
                current_month_str
            )  # Add to global set for final merging
            ...
            if month_data_exists:
                ...
                eod_month_df = month_subset.drop_duplicates(
                ...
                if previous_month_end_balance is not None and pd.isna(
                    eod_month_df.loc[0, "Balance"]
                ):
                    first_valid_idx = eod_month_df["Balance"].first_valid_index()
                    if first_valid_idx is not None:
                        eod_month_df.loc[0 : first_valid_idx - 1, "Balance"] = (
                ...
                fill_value = (
                    0.0
                    if previous_month_end_balance is None
            ...
            result_eod_bank = pd.concat(
                [result_eod_bank, eod_month_df], ignore_index=True
            )
        if result_eod_bank.empty:
            continue  # Skip if bank had no processable data at all
        ...
        pivot_numeric_cols = pivot_df.columns.difference(
            [day_col_name]
        )  # Exclude 'Day'/'Date'
        ...
        for col in pivot_numeric_cols:
            try:
                ...
                print(
                    f"Could not parse column '{col}' to determine days in month during zeroing.",
                    UserWarning,
                )
        pivot_df.loc[:, pivot_numeric_cols] = pivot_df.loc[:, pivot_numeric_cols].mask(
            mask, 0.0
        )
        if global_max_date and not pivot_df.empty:
            ...
            try:
                ...
                global_max_month_dt = pd.Timestamp(
                    global_max_date.year, global_max_date.month, 1
                )
                if last_month_dt == global_max_month_dt:
                    end_day_of_global_max = global_max_date.day
                    pivot_df.loc[
                        pivot_df[day_col_name] > end_day_of_global_max,
                        last_month_in_pivot,
                    ] = 0.0
            except ValueError:
                print(
                    f"Could not parse last column '{last_month_in_pivot}' during final day zeroing.",
                    UserWarning,
                )
    ...
    if len(multiple_eods) == 1:
        ...
        adf = process_repeating_columns(
            multiple_eods
        )  # Make sure this function correctly merges based on 'Day'
    final_ordered_months = sorted(
        list(all_banks_processed_months),
        key=lambda x: pd.to_datetime(x, format="%b-%Y", errors="coerce"),
    )
    day_col_name = (
        "Day" if "Day" in adf.columns else ("Date" if "Date" in adf.columns else None)
    )
    if day_col_name:
    ...
    numeric_cols_for_sum = (
        adf.columns.difference([day_col_name]) if day_col_name else adf.columns
    )
...
def opening_and_closing_bal(edf):
    ...
    if not month_columns or edf.empty:
        warnings.warn(
            "Input DataFrame 'edf' is empty or has no valid month columns.", UserWarning
        )
    ...
    for month_str in month_columns:
        try:
            ...
            warnings.warn(
                f"Column '{month_str}' not found during closing balance calculation.",
                UserWarning,
            )
            closing_bal[month_str] = np.nan
        except Exception as e:
            warnings.warn(
                f"An unexpected error occurred calculating closing balance for {month_str}: {e}",
                UserWarning,
            )
            closing_bal[month_str] = np.nan
    ...
    for i, month in enumerate(ordered_months):
        if i == 0:
            warnings.warn(
                f"Using balance from Day 1 of month '{month}' in the input DataFrame "
                "as the Opening Balance for this first month. Ensure this value "
                "represents the start-of-day balance.",
                UserWarning,
            )
            try:
                ...
                warnings.warn(
                    f"Could not retrieve balance from input DataFrame for Day 1 of the first month ({month}). "
                    "Setting opening balance to NaN.",
                    UserWarning,
                )
                opening_bal[month] = np.nan
            except (ValueError, TypeError):
                warnings.warn(
                    f"Balance value ('{first_day_balance_val}') from input DataFrame for Day 1 "
            ...
            if prev_month in closing_bal and not pd.isna(closing_bal[prev_month]):
                ...
                warnings.warn(
                    f"Could not find a valid closing balance for the previous month ({prev_month}) "
                    f"to use as opening balance for {month}. Setting opening balance to NaN.",
                    UserWarning,
                )
                opening_bal[month] = np.nan
...
def calculate_fixed_day_average(data):
    ...
    for month_str in month_columns:
        try:
            month_dt = pd.to_datetime(month_str, format="%b-%Y")
            days_in_month = month_dt.days_in_month
            valid_day_data = calc_df.loc[
                calc_df[day_col_name] <= days_in_month, month_str
            ]
    ...
    for day_set in sets_of_days:
        ...
        for month_str in month_columns:
            try:
                month_dt = pd.to_datetime(month_str, format="%b-%Y")
                days_in_month = month_dt.days_in_month
                valid_selected_days = selected_days_subset[
                    selected_days_subset[day_col_name] <= days_in_month
                ]
...
def process_avg_last_6_months(data, eod):
    ...
    if "Avg_Last_6_Months" in data.columns and len(data["Avg_Last_6_Months"]) > 1:
        ...
        print("'Avg_Last_6_Months' column does not exist or does not have enough data.")
        avg_divided_by_1_5 = np.nan
...
def category_add_ca(df):
    ...
    pos_pattern = r"^pos.*"
    df.loc[
        (df["Description"].str.contains(pos_pattern, regex=True)) & (df["Debit"] > 0),
        "Category",
    ] = "POS-Dr"
    df.loc[
        (df["Description"].str.contains(pos_pattern, regex=True)) & (df["Credit"] > 0),
        "Category",
    ] = "POS-Cr"

    pos_pattern_2 = r"^(vps|ips|ecom|pur|pcd|edc|ecompur)"
    df.loc[
        (df["Description"].str.contains(pos_pattern_2, regex=True)) & (df["Debit"] > 0),
        "Category",
    ] = "POS-Dr"
    df.loc[
            ...
            try:
                ...
                if name_match:
                    return name_match.group(
                        0
                    ).strip()  # Cleaned Name (without slashes/spaces)
                else:
    ...
    NEFT_ipay = df[
        df["Description"].str.contains("ipay/inst/neft/", case=False, na=False)
    ].copy()
    ...
    if not NEFT_HDFC_DR.empty:
        ...
        def extract_category(description):
            try:
                return description.split("-")[2] if "-" in description else description
            except IndexError:
                return "Suspense"
    ...
    def filter_emi_transactions(df):
        ...
        repeated_emi = repeated_emi[
            ~repeated_emi["Category"].str.contains("Investment", na=False)
        ]
    ...
    mask_withdrawal = (
        df["Description"].str.contains("eaw-|nwd-|atw-|tocash", case=False, na=False)
    ) & (df["Debit"] > 0)
    ...
    df.loc[
        df["Description"].apply(
            lambda x: any(keyword in x.lower() for keyword in online_shopping_keywords)
    ...
    INB = INB[
        ~INB["Category"].str.contains("Salary Paid|Salary Received|GST Paid", na=False)
    ]
    if not INB.empty:
    ...
    if not NEFT_INB.empty:
        NEFT_INB["Category"] = NEFT_INB["Description"].apply(
            lambda x: (
                x.split("/")[3] if "inb/neft" in x and len(x.split("/")) > 3 else x
            )
        )
        df.update(NEFT_INB)
    ...
    if not ECS.empty:
        ECS = ECS[
            ~ECS["Category"].str.contains("Redemption|Dividend & Interest", na=False)
        ]
    ...
    IMPS_HDFC = IMPS_HDFC[
        ~IMPS_HDFC["Category"].str.contains("Salary Paid|Salary Received", na=False)
    ]
    ...
    imps_rib = imps_rib[
        ~imps_rib["Category"].str.contains("Salary Paid|Salary Received", na=False)
    ]
    ...
    imps_mob = imps_mob[
        ~imps_mob["Category"].str.contains("Salary Paid|Salary Received", na=False)
    ]
    ...
    imps_fed = imps_fed[
        ~imps_fed["Category"].str.contains("Salary Paid|Salary Received", na=False)
    ]
    ...
    BRN_clg = df[df["Description"].str.contains("brn-clg-chqpaidto", na=False)]
    BRN_clg = BRN_clg[
        ~BRN_clg["Category"].str.contains("Salary Paid|Salary Received", na=False)
    ]
    if not BRN_clg.empty:
        BRN_clg["Category"] = BRN_clg["Description"].apply(extract_brn_clg_name)
        df.update(BRN_clg)
    ...
    RTGS_DCB = RTGS_DCB[
        ~RTGS_DCB["Category"].str.contains("Salary Paid|Salary Received", na=False)
    ]
    ...
    def extract_trtr_imps_category(description):
        try:
            ...
            if (
                len(parts) >= 4
                and parts[0].lower() == "trtr"
                and parts[2].lower() == "imps"
            ):
    ...
    imps_trtr = df[
        df["Description"].str.contains(r"trtr/\d+/imps/", case=False, na=False)
    ].copy()
    imps_trtr = imps_trtr[
        ~imps_trtr["Category"].str.contains(
            "Salary Paid|Salary Received", case=False, na=False
        )
    ]
    if not imps_trtr.empty:
        imps_trtr["Category"] = imps_trtr["Description"].apply(
            extract_trtr_imps_category
        )
    ...
    imps_in_df = df[
        df["Description"].str.contains(r"imps-in/\d+/\d+/", case=False, na=False)
    ].copy()
    imps_in_df = imps_in_df[
        ~imps_in_df["Category"].str.contains(
            "Salary Paid|Salary Received", case=False, na=False
        )
    ]
    if not imps_in_df.empty:
        imps_in_df["Category"] = imps_in_df["Description"].apply(
            extract_imps_in_category
        )
        df.update(imps_in_df)
    ...
    neft_in_df = df[
        df["Description"].str.contains(r"neft_in:ioban\d+/\d+/", case=False, na=False)
    ].copy()
    neft_in_df = neft_in_df[
        ~neft_in_df["Category"].str.contains(
            "Salary Paid|Salary Received", case=False, na=False
        )
    ]
    if not neft_in_df.empty:
        neft_in_df["Category"] = neft_in_df["Description"].apply(
            extract_neft_in_category
        )
    ...
    def extract_imps_io_category(description):
        try:
            ...
            if (
                len(parts) >= 4
                and parts[0].lower() == "imps"
                and parts[1] in ["in", "out"]
            ):
    ...
    imps_io_df = df[
        df["Description"].str.contains(r"imps/(in|out)/\d+/", case=False, na=False)
    ].copy()
    imps_io_df = imps_io_df[
        ~imps_io_df["Category"].str.contains(
            "Salary Paid|Salary Received", case=False, na=False
        )
    ]
    if not imps_io_df.empty:
        imps_io_df["Category"] = imps_io_df["Description"].apply(
            extract_imps_io_category
        )
    ...
    df["Voucher type"] = ""
    df.loc[
        df["Category"].isin(["Cash Withdrawal", "Cash Deposits"]), "Voucher type"
    ] = "Contra"
    df.loc[
        (df["Debit"] > 0)
        & (~df["Category"].isin(["Cash Withdrawal", "Cash Deposits"])),
        "Voucher type",
    ] = "Payment"
    df.loc[
        (df["Credit"] > 0)
...
def append_to_excel(file_path, new_data):
    ...
    if os.path.exists(file_path):
        ...
        with pd.ExcelWriter(
            file_path, mode="a", if_sheet_exists="overlay", engine="openpyxl"
        ) as writer:
            # Read existing sheet
...
def make_summary_great_again(df1, opening_closing_balance, df2):
    def generate_summary(table, value_column, summary_name):
        ...
        summary = summary.reindex(
            sorted(summary.columns, key=lambda x: pd.to_datetime(x, format="%b-%Y")),
            axis=1,
        )
    ...
    def get_missing_balances(opening_closing_balance, all_months):
        ...
        balances = {
            month: opening_closing_balance.get(month, ["", ""]) for month in all_months
        }
        ...
        for month in all_months:
            ...
            if opening == "":  # Fill missing opening balance
                balances[month][0] = (
                    previous_balance if previous_balance is not None else ""
                )
            if closing == "":  # Fill missing closing balance
                ...
                for next_month in all_months[all_months.index(month) + 1 :]:
                    if balances[next_month][0] != "":
                        ...
                        break
                balances[month][1] = (
                    next_opening if next_opening is not None else balances[month][0]
                )
            previous_balance = balances[month][1]

        # Sort results for output
        balances = dict(
            sorted(balances.items(), key=lambda x: pd.to_datetime(x[0], format="%b-%Y"))
        )
        return balances
    ...
    def get_particulars(df1, balances):
        ...
        summary["opening_balance"] = summary["Month-Year"].map(
            lambda x: balances.get(x, [0, 0])[0]
        )
        summary["closing_balance"] = summary["Month-Year"].map(
            lambda x: balances.get(x, [0, 0])[1]
        )
        ...
        sorted_date_columns = sorted(
            date_columns, key=lambda x: pd.to_datetime(x, format="%b-%Y")
        )
    ...
    category_add_rows = pd.DataFrame(
        {
            "Value Date": [pd.to_datetime(df1["Value Date"], format="%d-%m-%Y").min()]
    ...
    df1["Month-Year"] = pd.to_datetime(
        df1["Value Date"], format="%d-%m-%Y"
    ).dt.strftime("%b-%Y")
    ...
    all_month_years = (
        pd.date_range(
            start=min(
                [
                    pd.to_datetime(key, format="%b-%Y")
                    for key in opening_closing_balance.keys()
                ]
            ),
            end=max(
                [
    ...
    template_rows = pd.DataFrame(
        {
            "Value Date": [
                pd.to_datetime("01-" + month_year, format="%d-%b-%Y").strftime(
                    "%d-%m-%Y"
                )
    ...
    new_opening_closing_balance = get_missing_balances(
        opening_closing_balance, all_month_years
    )
    ...
    income_table = df1[
        df1["Category"].isin(df2[df2["Particulars"] == "Income"]["Category"])
    ]
    important_table = df1[
        df1["Category"].isin(
            df2[df2["Particulars"] == "Important Expenses / Payments"]["Category"]
        )
    ]
    other_table = df1[
        df1["Category"].isin(
            df2[df2["Particulars"] == "Other Expenses / Payments"]["Category"]
        )
    ]
    contra_credit_table = df1[
        df1["Category"].isin(df2[df2["Particulars"] == "Contra Credit"]["Category"])
    ]
    contra_debit_table = df1[
        df1["Category"].isin(df2[df2["Particulars"] == "Contra Debit"]["Category"])
    ]
    ...
    contra_credit_table = pd.concat([template_rows, contra_credit_table]).reset_index(
        drop=True
    )
    contra_debit_table = pd.concat([template_rows, contra_debit_table]).reset_index(
        drop=True
    )

    # Converting the "Value Date" column to datetime format in income_table, important_table, and other_table
    income_table["Value Date"] = pd.to_datetime(
        income_table["Value Date"], format="%d-%m-%Y"
    )
    important_table["Value Date"] = pd.to_datetime(
        important_table["Value Date"], format="%d-%m-%Y"
    )
    other_table["Value Date"] = pd.to_datetime(
        other_table["Value Date"], format="%d-%m-%Y"
    )
    contra_credit_table["Value Date"] = pd.to_datetime(
        contra_credit_table["Value Date"], format="%d-%m-%Y"
    )
    contra_debit_table["Value Date"] = pd.to_datetime(
        contra_debit_table["Value Date"], format="%d-%m-%Y"
    )

    income_table = income_table.sort_values(by=["Value Date"]).reset_index(drop=True)
    important_table = important_table.sort_values(by=["Value Date"]).reset_index(
        drop=True
    )
    other_table = other_table.sort_values(by=["Value Date"]).reset_index(drop=True)
    contra_credit_table = contra_credit_table.sort_values(
        by=["Value Date"]
    ).reset_index(drop=True)
    contra_debit_table = contra_debit_table.sort_values(by=["Value Date"]).reset_index(
        drop=True
    )

    income_summary = generate_summary(income_table, "Credit", "Income / Receipts")
    important_summary = generate_summary(
        important_table, "Debit", "Important Expenses / Payments"
    )
    other_summary = generate_summary(other_table, "Debit", "Other Expenses / Payments")
    contra_credit_summary = generate_summary(
        contra_credit_table, "Credit", "Contra Credit"
    )
    contra_debit_summary = generate_summary(contra_debit_table, "Debit", "Contra Debit")

    missing_months_list = get_missing_months(
        opening_closing_balance, new_opening_closing_balance
    )
    ...
    return (
        ...
        important_summary,
        other_summary,
        contra_credit_summary,
        contra_debit_summary,
        missing_months_list,
    )
...
def summary_sheet(idf, open_bal, close_bal, new_tran_df, new_categories=None):
    ...
    opening_closing_balance = {
        month: [open_bal[month], close_bal[month]] for month in open_bal
    }
    ...
    sheet_1, sheet_2, sheet_3, sheet_4, sheet_5, sheet_6, missing_months_list = (
        make_summary_great_again(new_tran_df, opening_closing_balance, df2)
    )
...
def creditor_list(df):
    ...
    Creditor_list = debit[debit["Description"].str.contains(regex_pattern, case=False)]
    ...
    name_transactions["Category"] = name_transactions["Description"].apply(extract_name)
...
def debtor_list(df):
    ...
    regex_pattern = "|".join(patterns)
    Debtor_list = credit[credit["Description"].str.contains(regex_pattern, case=False)]
    ...
    Debtor_list = Debtor_list[
        ~Debtor_list["Category"].str.contains("|".join(exclude_categories), case=False)
    ]
    ...
    def apply_regex_to_categories_hdfc(row):
        if row["Category"] in categories_to_include:
            if "upi-" in row["Description"]:
                ...
                match2 = re.search(
                    r"upi-\d+-([a-zA-Z][a-zA-Z0-9._]*)[-@]", row["Description"]
                )
...
def Eligibility_note(
    ...
    try:
        ...
        if not target_sheet:
            print(f"No sheet containing '{sheet_name_keyword}' found in the workbook.")
            return
        ...
        merge_end = f"H{start_row + 5}"  # Extend the merge to 5 rows below the start
...
def Summary_note(filename, empty_rows_between=1, sheet_name="Summary"):
    ...
    try:
        ...
        merge_start = f"A{start_row}"
        merge_end = f"H{start_row + 4}"  # Extend the merge to 3 rows below the start
...
def Investment_note(filename, empty_rows_between=1, sheet_name="Investment"):
    ...
    try:
        ...
        merge_start = f"A{start_row}"
        merge_end = f"H{start_row + 2}"  # Extend the merge to 3 rows below the start
...
def Emi_note(filename, note, empty_rows_between=1, column=1, sheet_name="Probable EMI"):
    note = note.replace("\n", "\r\n")
    try:
        workbook = openpyxl.load_workbook(filename)
        if sheet_name not in workbook.sheetnames:
...
def Suspense_Credit_note(filename, empty_rows_between=1, sheet_name="Suspense Credit"):
    note_parts = [
...
def Suspense_Debit_note(filename, empty_rows_between=1, sheet_name="Suspense Debit"):
    note_parts = [
        ...
        for index, row in df.iterrows():
            if row.isnull().all():  # Blank row indicates the end of a table
                if current_table_data:  # Process the current table if data exists
                    ...
                    table_df.columns = make_columns_unique(
                        table_df.iloc[0]
                    )  # Use the first row as headers
                    table_df = table_df[1:]  # Remove the headers from the data
                    table_df.reset_index(drop=True, inplace=True)
                    tables[f"Table {table_count}"] = json.loads(
                        table_df.to_json(orient="records")
                    )
    ...
    for sheet_name in excel_data.sheet_names:
        ...
        if sheet_name == "Summary":  # Special case for the "Summary" sheet
            ...
            for index, table in enumerate(tables):
                ...
                sheet_result[index_plus_one] = json.loads(
