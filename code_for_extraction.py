...
class ExtractionOnly:
    def __init__(self, bank_name, pdf_path, pdf_password, CA_ID, saved_pdf_file_code = "CA8104E"):
        self.writer = None
    ...
    def unlock_and_add_margins_to_pdf(self, pdf_path, pdf_password, timestamp, CA_ID):
        margin = 0.3
        os.makedirs(f"saved_pdf/pdf_{self.saved_pdf_file_code}", exist_ok=True)
        ...
        try:
            ...
            # Define the output path for the unlocked PDF
            unlocked_pdf_filename = f"{timestamp}-{CA_ID}.pdf"
            unlocked_pdf_path = os.path.join(f"saved_pdf/pdf_{self.saved_pdf_file_code}", unlocked_pdf_filename)

            # MARGIN CODE STARTS NOW: Convert margin from inches to points (1 inch = 72 points)
            margin_pts = margin * 72

            # Process the first page for trimming if needed
            cropped_doc = self.load_new_first_page_function(pdf_document)
    ...
    def new_mode_for_pdf(self, df, lists):
        print(df.head(20))
        # Check and remove common element from "Credit Column" and "Debit Column"
        if len(lists[2]) == 2 and len(lists[3]) == 2:
            common_element = set(lists[2]).intersection(lists[3])
        ...
        if lists[2][0] == lists[3][0]:
            print("Credit and Debit are in the same column.")
            result = self.credit_debit(df, lists[1], lists[0], lists[4], lists[2][0])
            final_df = self.cleaning(result)
        else:
            final_df = self.cleaning(new_df)
    ...
    def add_row_separators_in_memory(self, input_pdf_path):
        line_threshold = 5
        output_pdf_path = os.path.join(f"saved_pdf/pdf_{self.saved_pdf_file_code}", f"{self.CA_ID}_with_rows_added_{uuid.uuid4().hex}.pdf")
    ...
    def add_column_separators_in_memory(self, page):
        ...
        output_pdf, coordinates = self.process_pdf_and_annotate(page, os.path.join(f"saved_pdf/pdf_{self.saved_pdf_file_code}",
    ...
    def add_column_separators_with_coordinates(self, pdf_path, coordinates):
        ...
        processed_pdf_path = os.path.join(f"saved_pdf/pdf_{self.saved_pdf_file_code}", f"{self.CA_ID}_{uuid.uuid4().hex}_columns_adding_with_coordinates.pdf")
    ...
    def run_test_case_A(self, page):
        try:
            df = self.extract_dataframe_from_pdf(page, table_settings={
    ...
    def run_test_case_B(self, page_with_rows_added):
        try:
            df = self.extract_dataframe_from_pdf(page_with_rows_added, table_settings={
                "vertical_strategy": "lines",
    ...
    def run_test_case_C(self, page_with_columns_added):
        try:
            df = self.extract_dataframe_from_pdf(page_with_columns_added, table_settings={
                "vertical_strategy": "lines",
                "horizontal_strategy": "lines",
                "edge_min_length": 40,
            })
    ...
    def run_test_case_D(self, page_with_rows_n_columns_added):
        try:
            ...
            df = self.extract_dataframe_from_pdf(page_with_rows_n_columns_added, table_settings={
                "vertical_strategy": "lines",
                "horizontal_strategy": "text",
                "edge_min_length": 40,
                "intersection_x_tolerance": 120,
    ...
    def process_pdf_with_test_cases(self, pdf_path):
        ...
        # Test Case A
        model_df_A, lists = self.run_test_case_A(page)
        if model_df_A is not None:
            print("Test Case A passed")
            return ["A", 0, lists]  # Test Case A passed

        # Test Case B
        # page_with_rows = self.add_row_separators_in_memory(page)
        model_df_B, lists = self.run_test_case_B(page)
        if model_df_B is not None:
            print("Test Case B passed")
            return ["B", 0, lists]  # Test Case B passed

        # Test Case C
        page_with_columns, coordinates_C = self.add_column_separators_in_memory(page)
        model_df_C, lists = self.run_test_case_C(page_with_columns)
        if model_df_C is not None:
            print("Test Case C passed")
            return ["C", coordinates_C, lists]  # Test Case C passed

        # Test Case D
        # page_with_rows = self.add_row_separators_in_memory(page)
        page_with_columns_n_rows = self.add_column_separators_with_coordinates(page, coordinates_C)
    ...
    def run_test_output_on_whole_pdf(self, list_a, pdf_in_saved_pdf, bank_name, timestamp, CA_ID):
        ...
        if test_case == "A":
            ...
            df = self.extract_dataframe_from_pdf(pdf_in_saved_pdf, table_settings={
                "vertical_strategy": "lines",
                "horizontal_strategy": "lines",
                "edge_min_length": 30,
            })
            ...
            df = self.extract_dataframe_from_pdf(pdf_in_columns_saved_pdf, table_settings={
                "vertical_strategy": "lines",
                "horizontal_strategy": "lines",
                "edge_min_length": 40,
            })
