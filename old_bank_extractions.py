...
class CustomStatement:
    ...
    def add_lines_to_pdf(self, input_pdf_path, output_pdf_path, timestamp):
        ...
        for page in input_pdf.pages:
            ...
            for y_position in range(
                0, int(page_height), 6
            ):  # 1 cm = 28.35 points (assuming 1 inch = 72 points)
    ...
    def display_value_date_str(self, df):
        ...
        df["Value Date"] = (
            df["Value Date"].str.split(" ").str[0].str.split("-").str[2]
            + "-"
    ...
    def extract_the_df(self, idf):
        balance_row_index = idf[
            idf.apply(
                lambda row: "balance" in " ".join(row.astype(str)).lower(), axis=1
            )
        ].index
    ...
    def uncontinuous(self, df):
        df = df[
            ~df.apply(
                lambda row: row.astype(str).str.contains("Balance", case=False)
            ).any(axis=1)
        ]
        return df
    ...
    def unlock_the_pdfs_path(self, pdf_path, pdf_password, bank_name, timestamp):
            ...
            if pdf_reader.is_encrypted:
                ...
                try:
                    ...
                    raise ExtractionError(
                        "Incorrect password. Unable to unlock the PDF."
                    )
            else:
        ...
        unlocked_pdf_path = self.add_top_line_to_pdf(
            unlocked_pdf_path, unlocked_pdf_path
        )
        ...
        try:
            ...
            def idbi_format_1(unlocked_pdf_path):
                try:
                    ...
                    df = df.drop(
                        df.columns[0:2], axis=1
                    )  # Removes column at position 1
                    df = df.rename(
                        columns={
                            2: "Value Date",
                            3: "Description",
                    ...
                    df["Credit"] = 0
                    df["Debit"] = 0
                    df.loc[df["CR/DR"] == "Cr.", "Credit"] = df.loc[
                        df["CR/DR"] == "Cr.", "Amount (INR)"
                    ]
                    df.loc[df["CR/DR"] != "Cr.", "Debit"] = df.loc[
                        df["CR/DR"] != "Cr.", "Amount (INR)"
                    ]
                    df = df.drop(["CR/DR", "Amount (INR)"], axis=1)
                    # Replace '/' with '-' in the 'Value Date' column
                    df["Value Date"] = (
                        df["Value Date"].astype(str).str.replace("/", "-", n=2)
                    )
                    # date
            ...
            def idbi_format_2(unlocked_pdf_path):
                try:
                    unlocked_pdf_path = self.separate_lines_in_pdf_idbi(
                        unlocked_pdf_path, timestamp
                    )
                    pdf = pdfplumber.open(unlocked_pdf_path)
                    df_total = pd.DataFrame()
                    ...
                    df["Description"] = df.groupby("new_column")[
                        "Description"
            ...
            logger.error(
                f"An error occurred while processing pdf: {str(e)}", exc_info=True
            )
    ...
    def sbi(self, unlocked_pdf_path, timestamp):
        ...
        try:
            ...
            # date
            df["Value Date"] = pd.to_datetime(
                df["Value Date"], format="%d %b %Y", errors="coerce"
            ).dt.strftime("%d-%m-%Y")
            df = self.check_date(df)
        ...
        try:
            ...
            def yes_format_2(unlocked_pdf_path):
                ...
                try:
                    ...
                    def extract_specific_table_adjusted(pdf_path):
                        ...
                        with pdfplumber.open(pdf_path) as pdf:
                            ...
                            for page_num in range(total_pages):
                                ...
                                for table in tables:
                                    cleaned_columns = [
                                        col.strip() if col else "" for col in table[0]
                                    ]
                                    if set(desired_columns).issubset(
                                        set(cleaned_columns)
                                    ):
                                        df = pd.DataFrame(
                                            table[1:], columns=cleaned_columns
                                        )
                    ...
                    adjusted_specific_tables_df = extract_specific_table_adjusted(
                        unlocked_pdf_path
                    )
                    adjusted_specific_tables_df = adjusted_specific_tables_df.dropna(
                        axis=1, how="all"
                    )
        ...
        try:
            ...
            def union_format_1(unlocked_pdf_path):
                ...
                try:
                    ...
                    # start custom extraction
                    df = df.drop(
                        df.columns[[2, 3, 4]], axis=1
                    )  # Removes column at position 3,5
                    # df = df.iloc[1:] # Removes the first 1 rows
                    df = df.rename(
                        columns={
                            0: "Value Date",
                    ...
                    if " " in df["Value Date"].unique()[0]:
                        df["Value Date"] = pd.to_datetime(
                            df["Value Date"], format="%d-%m-%Y %H:%M:%S"
                        )
                        df["Value Date"] = df["Value Date"].dt.date
                        df["Value Date"] = pd.to_datetime(
                            df["Value Date"], format="%Y-%m-%d", errors="coerce"
                        ).dt.strftime("%d-%m-%Y")
                    else:
                        df["Value Date"] = pd.to_datetime(
                            df["Value Date"], format="%d/%m/%Y", errors="coerce"
            ...
            def union_format_2(unlocked_pdf_path):
                ...
                try:
                    ...
                    df["Credit"] = (
                        df["Transaction"]
                        .str.extract(r"(\d+\.\d+) \(Cr\)")
                        .astype(float)
                    )
                    df["Debit"] = (
                        df["Transaction"]
                        .str.extract(r"(\d+\.\d+) \(Dr\)")
                        .astype(float)
                    )
        ...
        try:
            ...
            def kotak_format_1(unlocked_pdf_path):
                try:
                    ...
                    one_pdf_path = self.separate_lines_in_pdf_idbi(
                        new_pdf_path, timestamp
                    )

                    x_positions = [20, 86, 260, 350, 480, 580, 660]
                    unlocked_pdf_path = self.separate_lines_in_vertical_pdf(
                        one_pdf_path, x_positions, timestamp
                    )
                    ...
                    df.loc[df["Amount"].str.contains("Cr"), "Credit"] = df.loc[
                        df["Amount"].str.contains("Cr"), "Amount"
                    ].str.replace(" Cr", "")
                    df.loc[df["Amount"].str.contains("Dr"), "Debit"] = df.loc[
                        df["Amount"].str.contains("Dr"), "Amount"
                    ].str.replace(" Dr", "")
    ...
    def bob(self, unlocked_pdf_path, timestamp):
        ...
        try:
            ...
            def bob_format_1(unlocked_pdf_path):
                ...
                unlocked_pdf_path = self.separate_lines_in_vertical_pdf(
                    unlocked_pdf_path, x_positions, timestamp
                )
            ...
            def bob_format_2(unlocked_pdf_path):
                ...
                new_pdf_path = self.separate_lines_in_vertical_pdf(
                    unlocked_pdf_path, x_positions, timestamp
                )
    ...
    def indus(self, unlocked_pdf_path, timestamp):
        ...
        try:
            ...
            def indus_format_3(unlocked_pdf_path):
                ...
                try:
                    unlocked_pdf_path = self.separate_lines_in_pdf(
                        unlocked_pdf_path, timestamp
                    )
                    print("Inside IndusInd Bank")
    ...
    def svc(self, unlocked_pdf_path, timestamp):
        ...
        try:
            ...
            logger.info(
                f"Extracted Dates from pdf; Start Date: {start_date_str}, End Date: {end_date_str}"
            )
            ...
            def further_refinement_parse_transaction_rows(extracted_text):
                ...
                for row in rows:
                    ...
                    if date_match:
                        numerical_values = [
                            match.group() for match in number_pattern.finditer(row)
                        ]
                        if numerical_values:
                            if current_particulars and parsed_data:
                                parsed_data[-1]["Particulars"] += (
                                    " " + current_particulars.strip()
                                )
                                current_particulars = ""
                            particulars = row[
                                date_match.end() : row.find(numerical_values[0])
                            ].strip()
                            parsed_data.append(
                                {
            ...
            complete_data = pd.DataFrame(
                columns=["Date", "Particulars", "Chq No", "Debit", "Credit", "Balance"]
            )
            ...
            with pdfplumber.open(unlocked_pdf_path) as pdf:
                for i in range(len(pdf.pages)):
                    ...
                    complete_data = complete_data._append(
                        pd.DataFrame(page_data), ignore_index=True
                    )
            ...
            start_date_datetime = pd.to_datetime(
                start_date_str, format="%d-%m-%Y"
            ).strftime("%d-%m-%Y")
            end_date_datetime = pd.to_datetime(
                end_date_str, format="%d-%m-%Y"
            ).strftime("%d-%m-%Y")
    ...
    def tdcb(self, unlocked_pdf_path, timestamp):
        ...
        try:
            ...
            logger.info(
                "PDF to table Extraction for Thane District Central bank completed"
            )
            return idf
    ...
    def bccb(self, unlocked_pdf_path, timestamp):
        ...
        try:
            ...
            logger.info(
                "PDF to table Extraction for Bassein Catholic Co-op bank completed"
            )
            return idf
    ...
    def scb(self, unlocked_pdf_path, timestamp, start_date):
        ...
        try:
            ...
            unlocked_pdf_path = self.separate_lines_in_pdf_scb(
                unlocked_pdf_path, timestamp
            )
            ...
            df["new_column"].fillna(method="ffill", inplace=True)
            df["Description"].fillna("", inplace=True)
            df["Description"] = df.groupby("new_column")["Description"].transform(
                lambda x: " ".join(x)
            )
            df = df.drop_duplicates(subset="new_column").reset_index(drop=True)
            df = df.drop(["new_column"], axis=1)
            ...
            df["Value Date"] = pd.to_datetime(
                df["Value Date"], format="%b %d %Y", errors="coerce"
            ).dt.strftime("%d-%m-%Y")
            ...
            df = df[
                ~(
                    (df["Debit"] > 1) & (df["Credit"] > 1)
                    | (pd.isna(df["Debit"]) & pd.isna(df["Credit"]))
                )
            ]
            df = df[df["Description"] != ""]
            df = df.dropna(subset=["Value Date"])
    ...
    def uco(self, unlocked_pdf_path, timestamp):
        ...
        try:
            ...
            def uco_format_2(unlocked_pdf_path):
                try:
                    unlocked_pdf_path = self.add_lines_to_pdf(
                        unlocked_pdf_path, unlocked_pdf_path
                    )
                    pdf = pdfplumber.open(unlocked_pdf_path)
                    df_total = pd.DataFrame()
        ...
        try:
            ...
            def saraswat_format_2(unlocked_pdf_path):
                try:
                    ...
                    for index, row in df.iterrows():
                        i = index
                        if (
                            "Cr" in df.at[i, "Transaction"]
                            or "Dr" in df.at[i, "Transaction"]
                        ):
            ...
            def saraswat_format_3(unlocked_pdf_path):
                try:
                    ...
                    df["Value Date"] = pd.to_datetime(
                        df["Value Date"], format="%d/%m/%Y %H:%M:%S", errors="coerce"
                    ).dt.strftime("%d-%m-%Y")
                    df = self.check_date(df)
                    df["Balance"] = df["Balance"].str.replace(
                        r"[^\d.-]+", "", regex=True
